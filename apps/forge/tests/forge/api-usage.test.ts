    ...
    for (const file of files) {
      ...
      for (const methodMatch of allMethodCalls) {
        ...
        const hasValidAuth =
          ...
          normalizedContext.includes(`getAuthForEvent(`); // abstraction over asUser/asApp

        expect(
          hasValidAuth,
          `File ${file}, line ${lineNum}: Use api.asUser().${methodName}() or api.asApp().${methodName}() or getAuthForEvent(...).${methodName}() - whitespace between method calls is allowed.\nLine: ${lineContent.trim()}`,
        ).toBe(true);
      }
    }
  });
